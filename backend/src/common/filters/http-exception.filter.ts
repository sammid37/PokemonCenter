...
export class HttpExceptionFilter implements ExceptionFilter {
  ...
  private handleDatabaseError(exception: QueryFailedError): {
    ...
    if (error.code === '23505') { // Violação de unique constraint no PostgreSQL
      const detail: string = error.detail ?? '';

      if (detail.includes('pokedexNumber')) {
        return {
          status: HttpStatus.CONFLICT,
          message: 'Já existe um pokémon com este número da Pokédex cadastrado por você',
        };
      }
    ...
    if (error.code === '23503') { // violação de foreign key no PostgreSQL
      return {
