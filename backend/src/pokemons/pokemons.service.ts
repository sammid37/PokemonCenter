...
export class PokemonsService {
  ...
  async update(id: string, dto: UpdatePokemonDto, user: string): Promise<Pokemon> {
    const pokemon = await this.findOne(id);
    
    if (pokemon.createdBy.id !== user) {
      throw new ForbiddenException('Você não tem permissão para editar este pokémon');
    }
  ...
  async remove(id: string, user: string): Promise<void> {
    ...
    if (pokemon.createdBy.id !== user) {
      throw new ForbiddenException('Você não tem permissão para excluir este pokémon');
    }
